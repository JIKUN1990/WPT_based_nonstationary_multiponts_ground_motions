function [outprm]=fn_getPARAM8_conditional(Mw,Rrup,Rhyp,Vs30,totalEnergytarget,filereg)
...
%     median prediction of each parameter
    ...
    hcoef=10;
    hcoef1=1;
    majorExM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*exp(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef1^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    majorEyM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*log(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    majorSxM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*exp(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef1^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    majorSyM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*log(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    majorRxyM_1 = acoef(ind) + bcoef(ind)*Mw + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;

    minorExM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*exp(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef1^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    minorEyM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*log(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    minorSxM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*exp(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef1^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    minorSyM_1  = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*log(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    minorRxyM_1 = acoef(ind) + bcoef(ind)*Mw + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    
    majorEaM_1     = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*log(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
    totalEnergyM_1 = acoef(ind) + bcoef(ind)*Mw + ccoef(ind)*log(Mw) + dcoef(ind)*D2 + ecoef(ind)*log(sqrt(D1^2+hcoef^2)) + fcoef(ind)*lAVS30; ind=ind+1;
   
    
    kexiE=(totalEnergytarget-totalEnergyM_1)/0.96;
    totalEnergyM=totalEnergytarget;
    majorExM = majorExM_1+kexiE*0.16*0.40;
    majorEyM = majorEyM_1+kexiE*0.09*0.48;
    majorSxM = majorSxM_1+kexiE*0.29*0.48;
    majorSyM = majorSyM_1+kexiE*0.23*0.68;
    majorRxyM=majorRxyM_1+kexiE*0.22*0.08
    
    minorExM = minorExM_1+kexiE*0.14*0.28;
    minorEyM = minorEyM_1+kexiE*0.15*0.44;
    minorSxM = minorSxM_1+kexiE*0.22*0.31;
    minorSyM = minorSyM_1+kexiE*0.26*0.49;
    minorRxyM=minorRxyM_1+kexiE*0.06*0.06;
    
    majorEaM = majorEaM_1+kexiE*0.87*1.33
